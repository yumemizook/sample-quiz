...
import vnLunar from 'https://esm.sh/vn-lunar';
...
async function getLunarInfo(date = new Date()) {
  try {
    ...
    // Some libraries use solar2lunar, others getLunarDate. 
    // vn-lunar (1.0.6) uses solar2lunar.
    const solar2lunar = vnLunar.solar2lunar || (vnLunar.default && vnLunar.default.solar2lunar) || vnLunar;

    if (typeof solar2lunar !== 'function' && typeof vnLunar.solar2lunar !== 'function') {
      // One last try: maybe it's the object itself with the method
      if (vnLunar && typeof vnLunar.solar2lunar === 'function') {
        const res = vnLunar.solar2lunar(y, m, d);
        return { ok: true, source: 'vn-lunar-client', ...res, tet: getTetInfo(date) };
      }
      throw new TypeError('solar2lunar is not a function on the imported module');
    }

    const res = typeof solar2lunar === 'function' ? solar2lunar(y, m, d) : vnLunar.solar2lunar(y, m, d);
    const tet = getTetInfo(date);

    // Ensure compatibility with index.html (which expects lunarMonth/lunarDay)
    return {
      ok: true,
      source: 'vn-lunar-client',
      ...res,
      lunarDay: res.lunarDay || res.day,
      lunarMonth: res.lunarMonth || res.month,
      lunarYear: res.lunarYear || res.year,
      tet
    };
  } catch (e) {
    console.warn('Lunar conversion local error:', e);
    // Fallback to mapping-based tet info
    try {
      const tet = getTetInfo(date);
      return { tet, source: 'mapping-fallback' };
    } catch (err) {
      return null;
    }
  }
